...
# ─────────────────────────────────────────────────────────────────────────────
#  MIXIN : visibilite conditionnelle selon les modules de l'org
# ─────────────────────────────────────────────────────────────────────────────

class ModuleGatedMixin:
    """
    Rend un ModelAdmin visible dans le menu de l'org-admin uniquement si
    le module correspondant (`org_module_flag`) est active pour son organisation.
    ...
    def has_delete_permission(self, request, obj=None):
        return request.user.is_superuser  # suppression reservee au superuser
...
class InterventionAdmin(ModuleGatedMixin, admin.ModelAdmin):
    ...
    search_fields = ['incident__titre', 'description', 'rapport']
...
# ─────────────────────────────────────────────────────────────────────────────
#  TYPE D'OUVRAGE ADMIN  (superuser uniquement — config GeoServer)
# ─────────────────────────────────────────────────────────────────────────────
...
class TypeOuvrageAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Couche cartographique — source de verite', {
            'description': (
        ...
        ('Champs manuels (heritage — a eviter)', {
            'classes': ('collapse',),
    ...
    @admin.display(description='Couche liee')
    def application_layer_link(self, obj):
        ...
        return format_html('<span style="color:#f59e0b;">Non liee</span>')
    ...
    def couche_geoserver_display(self, obj):
        if obj.application_layer:
            ...
            return format_html(
                ...
                ' <small style="color:#6b7f96;">(depuis la couche liee)</small>', val
            )
        return format_html('<span style="color:#6b7f96;font-style:italic;">Selectionnez une application_layer</span>')
