"""
sims_network.serializers
========================
Serialiseurs DRF pour Incident, Intervention, TypeIncident, Equipe, Photos et Ouvrages.
"""
...
class TypeOuvrageSerializer(serializers.ModelSerializer):
    class Meta:
        model  = TypeOuvrage
        fields = ['id', 'nom', 'couche_geoserver', 'layer_key', 'champ_cle', 'champ_nom', 'icone', 'actif', 'ordre']
...
class InterventionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'incident',
            'type_travaux', 'type_travaux_label', 'statut', 'statut_label',
            'description', 'rapport', 'observations',
            'responsable', 'responsable_detail', 'equipe', 'equipe_detail',
            'date_planifiee', 'date_debut', 'date_fin', 'duree_estimee', 'duree_reelle',
            'photos', 'created_by', 'created_by_detail', 'created_at', 'updated_at',
        ]
...
class InterventionListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = Intervention
        fields = ['id', 'type_travaux', 'type_travaux_label', 'statut', 'statut_label',
                  'responsable_detail', 'equipe_detail', 'date_planifiee', 'date_fin']
...
class IncidentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = Incident
        fields = [
            'id', 'ouvrage', 'ouvrage_detail', 'titre', 'description',
            'type_incident', 'type_incident_label', 'type_incident_detail',
            'statut', 'statut_label', 'priorite', 'priorite_label',
            'latitude', 'longitude', 'couche_id', 'couche_nom', 'feature_id',
            'localisation', 'quartier', 'ville',
            'signale_par', 'signale_par_detail', 'assigne_a', 'assigne_a_detail',
            'date_signalement', 'date_prise_charge', 'date_resolution',
            'created_at', 'updated_at', 'nb_interventions', 'interventions', 'photos',
        ]
        read_only_fields = ['created_at', 'updated_at', 'signale_par', 'signale_par_detail', 'nb_interventions']
    @extend_schema_field(OpenApiTypes.STR)
...
class IncidentListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = Incident
        fields = [
            'id', 'titre', 'ouvrage', 'ouvrage_detail',
            'type_incident', 'type_incident_label',
            'statut', 'statut_label', 'priorite', 'priorite_label',
            'latitude', 'longitude', 'localisation', 'quartier', 'ville',
            'signale_par_detail', 'assigne_a_detail',
            'date_signalement', 'date_resolution', 'nb_interventions',
        ]
    @extend_schema_field(OpenApiTypes.STR)
    def get_type_incident_label(self, obj):
        return obj.type_incident.nom if obj.type_incident else ''
...
class AssignerIncidentSerializer(serializers.Serializer):
    user_id = serializers.IntegerField(required=True, help_text="ID de l'utilisateur a assigner")
...
class CloturerInterventionSerializer(serializers.Serializer):
    rapport = serializers.CharField(required=False, allow_blank=True, default='',
                                    help_text='Rapport de cloture (optionnel)')
