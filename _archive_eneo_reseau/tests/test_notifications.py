...
class TestNotificationsFeedAvecSince:
    ...
    def test_incident_nouveau_apres_since(self, auth_client_lecteur):
        """Un incident créé après `since` → event type incident_nouveau."""
        since = self._since_iso(-2)
        inc = make_incident(titre="Incident récent")
    ...
    def test_intervention_nouvelle_apres_since(self, auth_client_lecteur):
        """Une intervention créée après `since` → event type intervention_nouvelle."""
        since = self._since_iso(-2)
        inc = make_incident(titre="Incident parent")
...
class TestNotificationsFeedStructureEvenements:
    ...
    def test_incident_nouveau_champs(self, auth_client_lecteur):
        """L'événement incident_nouveau contient les bons champs."""
        since = self._since_iso(-2)
        inc = make_incident(titre="Test champs nouveau")
    ...
    def test_incident_modifie_champs(self, auth_client_lecteur):
        """L'événement incident_modifie contient les bons champs."""
        inc = make_incident(titre="Test champs modifié")
        old_time = timezone.now() - timedelta(minutes=10)
        Incident.objects.filter(pk=inc.pk).update(created_at=old_time, updated_at=old_time)
    ...
    def test_intervention_nouvelle_champs(self, auth_client_lecteur):
        """L'événement intervention_nouvelle contient les bons champs."""
        since = self._since_iso(-2)
        inc = make_incident(titre="Incident pour intervention")
        iv = make_intervention(inc)
    ...
    def test_incident_titre_depuis_type_incident_si_titre_vide(self, auth_client_lecteur):
        """
